...
class Magestore_Affiliateplusstatistic_Model_Mysql4_Click_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    public function _construct(){
        parent::_construct();
        $this->_init('affiliateplusstatistic/click');
    }
    ...
    public function prepareSummary($range, $customStart, $customEnd, $isFilter = 0){
    	...
    	$this   ->getSelect()
                ->where('account_id = '.$account->getId().' AND type = 2')
                ->columns(array(
                    'clicks'	=> 'SUM(totals)',
                    'uniques'	=> 'SUM(is_unique)',
                ));
    	//Zend_Debug::dump($this->getSelect()->__toString());die('1');
