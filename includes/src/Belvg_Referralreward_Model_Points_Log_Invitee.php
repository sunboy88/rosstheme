...
class Belvg_Referralreward_Model_Points_Log_Invitee extends Belvg_Referralreward_Model_Points_Log_Abstract
{
    /**
     * Supplement Points
     *
    ...
    public function supplementPoints($order, $params = array())
    {
        ...
        $points = (int) $helper->storeConfig('points/invitee');
        if ($points) {
            $params['type']        = Belvg_Referralreward_Model_Points_Log::TYPE_CUSTOMER_INVITEE;
    ...
    public function withdrawPoints($order)
    {
        $orderLog   = Mage::getModel('referralreward/points_log')->getCollection()
            ...
            ->addFieldToFilter('status', Belvg_Referralreward_Model_Points_Log::TYPE_CUSTOMER_INVITEE)
            ->getFirstItem();
    ...
    {
        return Mage::helper('referralreward')->__('Invitee');
    }
